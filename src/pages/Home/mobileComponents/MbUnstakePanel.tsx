...
const MbUnstakePanel:FC = () => {
  ...
  const withDraw1 = async (bigNumStr: string) => {
    ...
    return new Promise(async resolve => {
      ...
      GET_DEP_CONTRACT.methods["withDraw"](bigNumStr)
          .send({from: account })
  ...
  const addQueue1 = async (bigNumStr: string) => {
    return new Promise(async resolve => {
      ...
      GET_EXIT_CONTRACT.methods["addQueue"](bigNumStr)
          .send({from: account })
          .on('receipt', (receipt: any) => {
            resolve({
              messageCode: 200,
    ...
    <div className='mb_claim_panel'>
      <div className='flexcss mb_claim_box'>
        ...
        <div className='mb_usp_main flexcss'>
          ...
          <p className='mb_usp_tips'>{intl('unstake.panel5', 'Staked balance:')}<span>{ formatUnits(get_user_staked_tron()) } TORN</span></p>
      ...
      <TorndoDialog title={intl('stake.modal4','Waiting For Confirmation')} visible={showWait} onClose={setShowWait}>
        <div className='trc_dialog st_dialog'>
          ...
          <div className='footer flexrcc'>
            <div className='btn primary' onClick={() => setShowWait(false)}>{intl('stake.modal.close', 'Close')}</div>
          </div>
        </div>
      </TorndoDialog>
      ...
      <TorndoDialog title={intl('stake.modal1', 'Insufficient balance')} visible={show} onClose={setShow}>
        <div className='trc_dialog st_dialog'>
          ...
          <div className='footer flexrcc'>
            <div className='btn primary' onClick={() => setShow(false)}>{intl('stake.modal.close', 'Close')}</div>
          </div>
        </div>
      </TorndoDialog>
    </div>
